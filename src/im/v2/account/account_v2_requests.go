...
type KickAccountRequestV2 struct {
	AccountId           string   `json:"-"`                               // 账号ID，用于构造URL
	Type                int      `json:"type,omitempty"`                  // 踢下线类型：1-踢所有设备，2-踢device_id_list中的设备，3-保留device_id_list中的设备，踢其他设备
...
type BusinessIdMapItem struct {
	Type       int    `json:"type,omitempty"`        // 类型：1-文本，2-图片
...
type UserInformation struct {
	...
	EmailValidationMode *int   `json:"email_validation_mode,omitempty"` // 用户信息校验模式，0-默认当前校验模式；1-扩展校验模式，包括特殊字符和拉丁字符；2-不校验，不建议用该模式，可能会导致未知显示问题
}
